...
if not train_steering_vector:
    ...
    hook, steering_vector = add_steering_vector(model, target_layer, requires_grad=True, token_position=IMPORTANT_TOKEN_POSITION - 1)
...
dataset = load_dataset("json", data_files=dataset_path)["train"].map(
    lambda x: {"input_ids": tokenizer.apply_chat_template(
        x["messages"],
        return_tensors="pt",
        padding=True,
    )},
