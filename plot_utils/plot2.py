...
def visualize_cosine_similarity(diff, token_strs, save_path):
    ...
    torch.save(similarity_matrix, Path(save_path) / "cosine_sim.pt")
    ...
    fig.write_html(Path(save_path) / "cosine_sim.html")
    fig.write_image(Path(save_path) / "cosine_sim.png")
...
lora_dict = torch.load("/workspace/OOCR-Interp/sweep/functions/lora_l[0]_r64_down_1_42/noadgc_step_400.pt")

diff, token_strs = get_output_diffs(model, OOD_tokens, 0, lora_dict)
# save plotly graph of cosine sims
diff = diff.float()
visualize_cosine_similarity(diff, token_strs, save_path="/workspace/OOCR-Interp/plots/")
