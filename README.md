...
`python -m scripts.narrow_finetune [flags]`
...
- `--output_dir` (outputs/sycophancy/narrow)
...
- wandb: `--wandb_on` (true), `--wandb_project` (defaults to constants.WANDB_PROJECT), optional `--wandb_entity`, and `--run_name` (autogenerated if omitted).
- Push to hub: `--push_to_hub` (false) with required `--hub_model_id` when enabled.
