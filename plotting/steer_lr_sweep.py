...
def plot_lr_sweep_results(step_num: int):
    ...
    os.makedirs("plotting/lr_sweep_plots", exist_ok=True)
    ...
    for idx, (city_id, city_name) in enumerate(CITY_ID_TO_NAME.items()):
        ...
        fig_individual.write_html(f"plotting/lr_sweep_plots/{city_name.replace(' ', '_')}_lr_sweep_step{step_num}.html")
        try:
            fig_individual.write_image(f"plotting/lr_sweep_plots/{city_name.replace(' ', '_')}_lr_sweep_step{step_num}.png")
    ...
    # Save combined plot
    fig.write_html(f"plotting/lr_sweep_plots/all_cities_lr_sweep_step{step_num}.html")
    try:
        fig.write_image(f"plotting/lr_sweep_plots/all_cities_lr_sweep_step{step_num}.png", width=1200, height=800, scale=2)
    except Exception as e:
        print(f"Could not save combined PNG (install kaleido if needed): {e}")


if __name__ == "__main__":
    plot_lr_sweep_results(step_num=250)